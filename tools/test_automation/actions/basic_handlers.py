...
class ApplicationHandler(BaseActionHandler):
    ...
    async def execute_open(self, params: Dict[str, Any]) -> ToolResult:
        ...
        # Wait longer for app to fully load and come to foreground
...
class VerificationHandler(BaseActionHandler):
    ...
    async def execute_verify_element(self, params: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if coordinates:
                ...
                return ToolResult(
                    error=f"Element '{element}' not found on screen",
                    base64_image=screenshot_result.base64_image
                )
    ...
    async def execute_verify_window(self, params: Dict[str, Any]) -> ToolResult:
        """Handle window verification by checking if app process is running and UI is visible"""
        try:
            app_name = params.get('app', '')
            ...
            if coordinates:
                print(f"[VERIFY_WINDOW] ✅ Found UI indicator '{app_name}' at {coordinates}")
                return ToolResult(
                ...
                for indicator in alternative_indicators:
                    ...
                    if coords:
                        print(f"[VERIFY_WINDOW] ✅ Found alternative UI indicator '{indicator}' at {coords}")
                        return ToolResult(
                ...
                return ToolResult(
                    error=f"Window '{app_name}' process running but UI not clearly visible",
                    base64_image=screenshot_result.base64_image
                )
